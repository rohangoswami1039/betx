...
const App = () => {
  ...
  const openPlayStore = () => {
    const playStoreLink = 'https://play.google.com/store/apps/details?id=com.betx&pcampaignid=web_share';
  ...
  useEffect(() => {
    ...
    StatusBar.setBackgroundColor('#101010');
  ...
  if(version && version !== DeviceInfo.getVersion()){
    return (
      <NativeBaseProvider>
      <View style={{ flex: 1, backgroundColor: '#181829', justifyContent: 'flex-start', alignItems: 'center' }}>
        <Image
          source={require('./assets/images/AppLogoSlogan.png')}
          alt="Alternate Text"
          style={{ width: '80%', maxWidth: 300, height: 200, resizeMode: 'contain' }}
        />
        <Actionsheet isOpen={true}>
          <Actionsheet.Content bg={'#222232'}>
            <View style={{ justifyContent: 'center', marginTop: 10, marginBottom: 20 }}>
              <Text style={{ color: 'white', fontSize: 25, fontWeight: 'bold' }}>New Update Available</Text>
              <View style={{flexDirection:'row',justifyContent:'center',marginTop:10}}>
                  <Text style={{color:'white',fontWeight:'bold'}} >Version :</Text>
                  <Text style={{color:'white',marginLeft:5}}>{version}</Text>
              </View>
            </View>
            <Text style={{ color: 'white', fontSize: 18,fontWeight:"bold" ,marginBottom: 20 }}>{version_message}</Text>
            <View>
              <Button size="lg" variant="outline" _text={{ color: "white",fontWeight:'bold' }} style={{
